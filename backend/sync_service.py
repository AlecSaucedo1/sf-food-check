...
async def sync_once(
    ...
    *,
    ...
    """Fetch a complete DataSF snapshot and atomically replace structured inspection rows.

    The existing database is left intact if the upstream fetch fails. Report enrichment
    is stored separately and is not deleted during a structured-data refresh.
    """
    started_at = utc_now()
    try:
        ...
        with connect(db_path) as con:
            ...
            record_sync_run(con, started_at=started_at, completed_at=completed_at, success=True, row_count=count, facility_count=facilities, latest_inspection_date=latest, error="")
        return {"ok": True, "rows": count, "facilities": facilities, "latest_inspection_date": latest, "started_at": started_at, "completed_at": completed_at}
    except Exception as exc:
        try:
            with connect(db_path) as con:
                record_sync_run(con, started_at=started_at, completed_at=utc_now(), success=False, row_count=0, facility_count=0, latest_inspection_date=None, error=str(exc)[:2000])
        except Exception:
            pass
        raise
